...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.get("/admin/export/chantiers.csv", { preHandler: [app.authenticate, requireRole(["ADMIN"])] }, async (req, reply) => {
    ...
    const headers = ["id", "nom", "client", "adresse", "montant", "statut", "createdAt", "updatedAt"];
    const rows = items.map(i => [i.id, i.nom, i.client, i.adresse, i.montant ?? "", i.statut ?? "", i.createdAt.toISOString(), i.updatedAt.toISOString()]);
    const csv = toCsvBomSemicolon(headers, rows);
  ...
  app.post("/admin/import/chantiers", { preHandler: [app.authenticate, requireRole(["ADMIN"])] }, async (req, reply) => {
    ...
    for (const r of rows.slice(1)) {
      ...
      const montantRaw = (idx("montant") >= 0 ? r[idx("montant")] : "") || "";
      const montant = montantRaw ? Number(montantRaw.replace(",", ".")) : undefined;

      const statut = (idx("statut") >= 0 ? r[idx("statut")] : undefined) || undefined;

      await prisma.chantier.create({
        data: { entrepriseId, nom, client, adresse, montant: Number.isFinite(montant) ? montant : undefined, statut }
      });
      created++;
    }
